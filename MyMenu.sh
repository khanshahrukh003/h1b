...
show_menu
    ...
    if [[ $opt = "" ]]; then 
        ...
        hive -e "select year,count(year) from niit.h1b_final where job_title LIKE '%DATA ENGINEER%' group by year; "	
        show_menu;
        ;;
        2) clear;
        option_picked "1 b) Find top 5 job titles who are having highest growth in applications. ";
        stop-all.sh
		pig -x local /home/srk/niit/Pig/question1b.pig
        show_menu;
        ;;  
        3) clear;
        option_picked "2 a) Which part of the US has the most Data Engineer jobs for each year?";
	        
	pig -x local /home/srk/niit/Pig/question2a.pig
        show_menu;
        ;;
	    4) clear;
		...
		read var
	    hive -e " select year, worksite,count(case_status) as total_case_status from niit.h1b_final where year =$var and case_status='CERTIFIED' group by worksite,year order by total_case_status desc limit 5;" 
        show_menu;
        ;;  
	    5) clear;
        option_picked "3) Which industry has the most number of Data Scientist positions?";
        start-all.sh
        hive -e "select soc_name,count(soc_name) as cnt from niit.h1b_final where job_title LIKE '%DATA SCIENTIST%' group by soc_name order by cnt desc;"
        show_menu;
        ;;
        6) clear;
        option_picked "4)Which top 5 employers file the most petitions each year?";
		restart-all.sh        
		hive -e "select employer_name,year,count(year) as cnt from niit.h1b_final group by year,employer_name order by cnt;"
        show_menu;
        ;;
        7) clear;
	    ...
	    hive -e "select job_title,year,count(case_status ) as temp from niit.h1b_final where year= $var group by job_title,year  order by temp desc limit 10; "
        show_menu;
        ;;
        8) clear;
       	stop-all.sh
		...
		pig -x local /home/srk/niit/Pig/question6.pig
        show_menu;
        ;;
		9) clear;
		start-all.sh
		...
		hive -e "select year,count(*) as applications  from niit.h1b_final where year like '201%' group by  year;"
        show_menu;
        ;;
		10) clear;
		...
		read var
        hive -e "select job_title,full_time_position,year,avg(prevailing_wage) as average from niit.h1b_final where full_time_position = '$var' and year = $year group by job_title,full_time_position,year order by average desc;"
        show_menu;
        ;;
		11) clear;
		stop-all.sh
		...
		pig -x local /home/srk/niit/Pig/question9.pig
        show_menu;
        ;;
		12) clear;
		stop-all.sh
		...
		stop-all.sh
		rm -r /home/srk/Pig/question10
		pig -x local /home/srk/niit/Pig/question10.pig
		cat /home/srk/niit/Pig/question10/p*
        show_menu;
        ;;
		13) clear;
		...
		start-all.sh
		sleep 10;
		bash /home/srk/niit/Sqoop/question11.sh
        show_menu;
        ;;
		\n) exit;   
		;;
